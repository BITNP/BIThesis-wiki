...
::: warning 与 TeX Live 2024 不兼容

由于 [LaTeX 内核方面的问题](https://github.com/CTeX-org/ctex-kit/issues/725)，在特定版本的 TeX Live 2024 上，cleveref 与 ctex 不兼容，无法使用。

您可考虑换用 hyperref 提供的`\autoref`，详见[`\autoref`中文引用名称](./autoref-name.md)；只不过`\autoref`功能略少，比如不支持在同一处引用多项。
:::
...
1. 在导言区**导入[[pkg]]**
   ...
   一般要在其之前导入[[pkg]]；不过对于[[pkg]]，模板已在最开始`\documentclass`处自动导入，无需重复。
...
2. **用`\label{…}`标记**要引用的内容
   ...
   ```latex
   \section{章节标题} \label{sec:intro} % [!code highlight]

   \begin{figure}[h]
     ……
     \caption{图表标题} \label{fig:example} % [!code highlight]
   \end{figure}
...
3. **用`\cref{…}`引用**
   ...
   ::: tip 🔍 数字与汉字间的空白
   [[pkg]]会自动在数字与汉字间加些许空白，但`图 3-5 可验证`的`5 可`位于宏的边界处，不会自动加。如果您介意这一点，建议手动空一格。
   :::
